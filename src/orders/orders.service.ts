    ...
    try {
      ...
      if (user.role === UserRole.Client) {
        orders = await this.orders.find({
          where: {
            customer: { id: user.id }, // Assuming 'user' is an instance of the User entity
            ...(status && { status }),
          },
        });
  ...
  async editOrder(user:User, {id:orderId, status}:EditOrderInput): Promise<EditOrderOutput>{
    try {
      ...
      if (user.role === UserRole.Delivery) {
        if(status !== OrderStatus.PickedType && status !== OrderStatus.Deliverd){
          canEdit = false
        }
      }
      if(!canEdit){
  ...
  async takeOrder(driver:User, {id:orderId}:TakeOrderInput): Promise<TakeOrderOutput>{
    try {
      ...
      if (order.driver) {
        return{
          ok:false,
          error: "This order already has an driver"
        }
      }
